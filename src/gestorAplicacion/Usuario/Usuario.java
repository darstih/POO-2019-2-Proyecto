...
public abstract class Usuario{
	...
	public static Obra obraConMasComentarios(){
		Obra obras=null;
		int mas=0;
		for(int i=0;i<Obra.getCantObras();i++) {
			if(Obra.getObras().get(i).devolverCantComentarios()>mas) {
				obras=Obra.getObras().get(i);
				mas=Obra.getObras().get(i).devolverCantComentarios();
			}
		}
		return obras;
	}
